...
| 时间 | 名称 | 内容 | 投递 | 状态 |
|------|------|------|------|------|
| 工作日 07:00 | Cask每日简报（工作日） | 📅黄历 + 🔧信号摘要 → 💪（每天一个思路） | Discord Home | ✅ |
...
- **脚本**: `cask_daily_brief.py`（只输出 `date` / `weekday` / `lunar`）
- **continuity**: ✅ 开（💪 靠它记住上一轮说过什么，同一切口 7 天内不重复）
- **结构**:
  1. 📅 黄历（lunar-python生成）
  2. 🔧 今日信号简报采集摘要（读 ~/Cask窗台/信号简报/raw/ 最新采集，提 2-3 条）
  3. 💪 我可以为你做什么 = **每天一个「思路」**（视角/切口/方法），不是待办、不是进度；取材 `~/Cask窗台/在意命题.md` + 当天信号 + 她最近在写/在说的事。落点是「我出活」（一页拆解/两版走向对比/对照表/口播稿），她答「行/不用」
- **💪 红线**: 禁进度与待办（"该做X了""等你N天了""待推进/顺延/落后"）、禁催未决项、禁鸡汤、**宁缺勿凑**（没切口就短写或不写）、一天最多 2 条（默认 1）
- **变更记录**: 2026-07-10 黑客松进度 → 今日神话/趣闻；2026-07-21 北欧连续剧模式；2026-08-04 铁律硬化（禁止过程元信息，正文逐字进Final Response）+ 修复黄历脚本路径bug（直接取Script Output注入数据）；2026-08-05 💪改为从看板读取（硬编码方向词 → 看板当前状态）；2026-08-11 砍掉神话模块（用户觉得没意思，等想到更有意思的再恢复）→ 工作日=黄历+信号摘要+💪；**2026-09-10 💪 重定义为「每天一个思路」**（她原话："本质上我是想让你提供一个思路方式，而不是催我做什么东西"）——不再读速览表挑待推进方向，新增命题池 `~/Cask窗台/在意命题.md`、开 continuity 做去重；同日清掉脚本里过期的 NMA 黑客松段（原在注入 `remaining_days: -63`）
