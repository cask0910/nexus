...
- **状态**：M1 ✅ + M2 ✅（2026-08-23）· **M3 = Vue 3 全面迁移 + 统一菜库 + 四角色**（定稿）
- **方案全文**：`~/forge/grandma-menu/docs/产品改造方案.md`（四角色入口、统一菜库两层模型、视觉①温馨暖灶双主题、M2 独立入口设计、M3 完整方案）
- **要点**：四角色 = 📋点菜 / 🍳做菜 / 💡找点灵感（不记忆）/ 🛠管菜；**统一菜库**：源库（599 道，助手同步）+ 自家库（管菜维护，支持 B站/抖音视频，两家共用，隐藏全局化）；点菜台只显示自家库；食材找菜 `/cook-find` 已上线；M4 = 开源源同步挂 cron
