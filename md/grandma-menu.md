...
- **状态**：M1 ✅ + M2 ✅（2026-08-23）· M3 待启动 · 触发：奶奶家食材找菜需求 + 现有 UI 整体改造
- **方案全文**：`~/forge/grandma-menu/docs/产品改造方案.md`（双角色入口、做菜端一页菜名+视频/备菜清单、视觉①温馨暖灶双主题、M2 独立入口设计）
- **要点**：角色=点菜人/做菜人；做菜端 `/cook` 已上线；**食材找菜=独立页面 `/cook-find`**（非 tab，tab 下方入口卡片 → 三步向导 → 总览/大字卡 → 详情，599 道 MIT 菜谱 + 自家优先，同步脚本 scripts/sync-recipes.py）；数据接入见 docs/食材找菜集成方案.md（M4 挂 cron）
