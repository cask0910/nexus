...
- **状态**：M1 ✅（2026-08-23）· M2 待启动 · 触发：奶奶家食材找菜需求 + 现有 UI 整体改造
- **方案全文**：`~/forge/grandma-menu/docs/产品改造方案.md`（双角色入口、做菜端一页菜名+视频/备菜清单、视觉①温馨暖灶双主题）
- **要点**：角色=点菜人/做菜人；做菜端 `/cook` 已上线（今天吃什么一页全菜名+真实BV视频链接、备菜清单⇄总览、空状态、10秒轮询）；角色入口 localStorage 记忆可切换；5 道内置菜补 video；数据接入 599 道 MIT 菜谱（细节见 docs/食材找菜集成方案.md，M4）
