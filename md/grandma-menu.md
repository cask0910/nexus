...
- **状态**：M1 ✅ + M2 ✅ + **UI 设计定稿 ✅**（2026-08-23）· **下一步 M3 = Vue 3 迁移**（点菜台美团式/底部tab、管菜 PC 响应式等预览全部确认）
- **方案全文**：`~/forge/grandma-menu/docs/产品改造方案.md`（UI 定稿清单、M3-M7 里程碑、四角色、统一菜库两层模型、管菜简单密码）
- **预览页**（线上可看）：`preview-order.html` 点菜台 · `preview-admin.html` 管菜 · `cook.html` 做菜端 · `cook-find.html` 食材找菜
- **要点**：四角色 = 📋点菜 / 🍳做菜 / 💡找点灵感（不记忆）/ 🛠管菜（简单密码）；**统一菜库**：源库（599 道，助手同步 M6）+ 自家库（管菜维护，支持 B站/抖音视频，两家共用，隐藏全局化）；点菜台只显示自家库；M3 只迁移架构不换数据模型，家人无感；M7 开源准备
