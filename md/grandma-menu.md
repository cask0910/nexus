...
- **状态**：M1 ✅ + M2 ✅（2026-08-23）· **M3 = Vue 3 全面迁移**（已重写方案）· 触发：奶奶家食材找菜需求 + 现有 UI 整体改造
- **方案全文**：`~/forge/grandma-menu/docs/产品改造方案.md`（双角色入口、做菜端一页菜名+视频/备菜清单、视觉①温馨暖灶双主题、M2 独立入口设计、M3 Vue3 迁移）
- **要点**：角色=点菜人/做菜人；做菜端 `/cook` 已上线；**食材找菜=独立页面 `/cook-find`**（599 道 MIT 菜谱 + 自家优先，同步脚本 scripts/sync-recipes.py）；**M3：Vue3+Vite MPA 全量迁移，共享 Header 组件（解决 cook/cook-find header 不一致）+ 食材找菜结果分组折叠（解决 254 道太多），点菜端换皮，并行重写不断服务**
